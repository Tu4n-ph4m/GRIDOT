...
def run_gridnet(X,Y,X_feature_names,Y_feature_names,candidate_XY_pairs,dag_adjacency_matrix,
			...
			train_separate=False,save_dir='./CRE_gene',save_name='gridot',
			verbose=True):
	...
	for k,result in results_dict.items():
		results_df[k] = result
	# results_df.to_csv(os.path.join(save_dir,'{}.gridot.tsv'.format(
...
def run_gridot_cell_population(rna_adata,atac_adata,
                               ...
                               seed = 1,
                               device = 'cpu'):
    print(f"Identifying candidate pairs within {distance_threshold} bp")
    candidates_df = identify_all_peak_gene_link_candidates(rna_adata,atac_adata,distance_thresh=distance_threshold)
    print(len(candidates_df))
    num_components = 50
    if len(rna_adata.obs)<50:
        num_components = len(rna_adata.obs)-1
    X_joint = schema_representations(rna_adata,atac_adata,schema_reference='rna',num_components=num_components)
    root = cell_root(rna_adata,marker = marker_gene)
    hops = 5

    dag_adjacency_matrix = construct_dag(X_joint,iroot=root,n_neighbors=15,pseudotime_algo='dpt')

    hop_matrix = n_hop_adjacency(dag_adjacency_matrix,hops)
    ...
    results = run_gridnet(X,Y,X_feature_names,Y_feature_names,
    ...
    results['dist'] = candidates_df['dist'].abs()
    create_folder(folder_path)
    outfile = f"{folder_path}/CRE_gene_cell_population.tsv"
